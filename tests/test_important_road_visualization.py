...
def render(
  ...
  if building_patches:
    col = PatchCollection(
      building_patches, facecolor="#aec6e8", edgecolor="none", alpha=0.25
    )
    ax.add_collection(col)

  if normal_patches:
    col = PatchCollection(
      normal_patches, facecolor="#cccccc", edgecolor="#999999", linewidth=0.2, alpha=0.5
    )
    ax.add_collection(col)
  ...
  legend_handles = [
    mpatches.Patch(
      facecolor="#e63946",
      edgecolor="#800000",
      label=f"Important road ({len(important_patches)})",
    ),
    mpatches.Patch(
      facecolor="#cccccc",
      edgecolor="#999999",
      label=f"Normal road ({len(normal_patches)})",
    ),
    mpatches.Patch(
      facecolor="#aec6e8", edgecolor="none", label=f"Building ({len(building_patches)})"
    ),
  ]
...
def test_important_road_visualization() -> None:
  ...
  output_path = ROOT / "tests/output/kobe1_important_roads.png"
  render(
    nodes, edges, roads, buildings, important_ids, output_path, gml_name=GML_PATH.name
  )
...
def main() -> None:
  parser = argparse.ArgumentParser(
    description="Visualize important roads on a GML map."
  )
  parser.add_argument(
    "gml",
    nargs="?",
